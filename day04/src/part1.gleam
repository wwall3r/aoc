...
fn xmas_check(grid: Grid, coord: Coord) -> Int {
  all_dirs
  |> list.map(fn(next_coord) {
    // this seems dumb but oh well
    let second = next_coord(coord)
    let third = next_coord(second)
    let fourth = next_coord(third)

    [coord, second, third, fourth]
    |> list.map(fn(coord) { get_value(grid, coord) })
    |> string.join("")
  })
...
const all_dirs = [
  get_left, get_right, get_up, get_down, get_left_up, get_left_down,
  get_right_up, get_right_down,
]

fn get_right(coord: Coord) {
  let #(x, y) = coord
  #(x + 1, y)
}

fn get_left(coord: Coord) {
  let #(x, y) = coord
  #(x - 1, y)
}

fn get_up(coord: Coord) {
  let #(x, y) = coord
  #(x, y - 1)
}

fn get_down(coord: Coord) {
  let #(x, y) = coord
  #(x, y + 1)
}
